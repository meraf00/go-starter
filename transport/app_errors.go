...
func ValidationError(code string, verr validator.ValidationErrors) *AppError {
	...
	appErr := New(BadRequestError, combinedMessage, code, nil, nil)
	appErr.AddMeta("fields", fieldErrors)
	return appErr
}

func Conflict(message, code string, err error) *AppError {
	return New(ConflictError, message, code, nil, err)
}
